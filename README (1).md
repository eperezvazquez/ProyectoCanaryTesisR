...
Prophet is an open source tool from Facebook used to forecast time series data that helps businesses understand and possibly predict the market. It is based on an additive decomposable model where non-linear trends are adjusted for seasonality, it also takes into account the effects of holidays.
